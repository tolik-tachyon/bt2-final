...
export function friendlyError(error: unknown): string {
    ...
    if (error instanceof BaseError) {
        ...
        if (short.includes("user rejected"))
            return "Transaction rejected in wallet.";
        if (short.includes("insufficient funds"))
            return "Insufficient native token balance for gas.";
        if (short.includes("insufficient balance"))
            return "Insufficient token balance.";
        if (short.includes("insufficient allowance"))
            return "Token approval is too low.";
        if (
            short.includes("chain mismatch") ||
            short.includes("wrong network")
        ) {
    ...
    if (error instanceof Error) {
        ...
        if (message.includes("failed to fetch"))
            return "Could not reach the subgraph endpoint.";
