...
export function StatCard({
    ...
    return (
        <div
            className={cn(
                "card-sm flex flex-col gap-1",
                accent && "border-accent/30 bg-accent/5",
            )}
        >
            <span className="stat-label">{label}</span>
            <span className={cn("stat-value", accent && "text-accent")}>
                {value}
            </span>
            {sub && (
                <span className="text-subtext text-xs font-mono">{sub}</span>
            )}
        </div>
    );
}
...
export function Badge({
    ...
    color?: "default" | "green" | "red" | "orange" | "purple" | "accent";
}) {
    const colors = {
        ...
        <section className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    {sub && (
                        <p className="text-subtext text-sm mt-0.5">{sub}</p>
                    )}
                </div>
                {action}
            </div>
...
export function InputField({
    ...
    return (
        <div className="flex flex-col gap-1.5">
            <label className="text-xs font-mono text-subtext uppercase tracking-widest">
                {label}
            </label>
            <input
                type={type}
            ...
            />
            {hint && (
                <span className="text-xs text-subtext font-mono">{hint}</span>
            )}
        </div>
    );
}
    ...
    if (!isConnected) {
        return (
            <div className="flex flex-col items-center justify-center gap-4 py-24 text-center">
                ...
                <p className="text-subtext font-body text-sm">
