...
export default function CraftPage() {
  ...
  const { data: balances } = useReadContracts({
    contracts: address && missing.length === 0 ? [1n, 2n, 3n, 4n, 5n, 6n].map((id) => ({
  ...
  return (
    <ConnectGuard>
      <main className="max-w-3xl mx-auto px-4 py-8 flex flex-col gap-6 animate-fade-in">
        <Section title="Crafting Workshop" sub="Burn Elements of Harmony to mint a pony NFT">
            ...
            {PONIES.map(({ id, name, icon }) => (
              <button
                ...
                className={`card text-left transition-all ${
                  selected === id ? "border-accent/60 bg-accent/5" : "hover:border-border/80"
                }`}
              >
              ...
              {Object.entries(recipe).filter(([, v]) => v > 0).map(([id, needed]) => {
                ...
                return (
                  <div key={id} className="flex items-center justify-between">
                    <span className={`text-sm font-mono ${ELEMENT_COLORS[id]}`}>{ELEMENT_NAMES[id]}</span>
          ...
          <div className="flex flex-col gap-3">
            {isSuccess ? (
              <Notice tone="success" message="Craft successful. Check your inventory." />
            ) : (
              <TxButton onClick={handleCraft} loading={isPending || isConfirming} disabled={!canCraft}>
                {isPending
                  ? "Confirm in wallet..."
                  : isConfirming
                    ? "Minting..."
