...
export default function VaultPage() {
  ...
  const { data, refetch } = useReadContracts({
    contracts: address && missing.length === 0 ? [
      ...
      { address: ADDRESSES.nftRentalVault,  abi: NFT_RENTAL_VAULT_ABI,  functionName: "boostedNftId" },
      ...
      { address: ADDRESSES.governanceToken, abi: GOVERNANCE_TOKEN_ABI,  functionName: "allowance",   args: [address, ADDRESSES.nftRentalVault] },
      { address: ADDRESSES.equestria1155,   abi: EQUESTRIA_1155_ABI,    functionName: "balanceOf",   args: [address, TOKEN_IDS.PINKIE_PIE] },
      { address: ADDRESSES.equestria1155,   abi: EQUESTRIA_1155_ABI,    functionName: "isApprovedForAll", args: [address, ADDRESSES.nftRentalVault] },
    ] : [],
  });
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 flex flex-col gap-6 animate-fade-in">
        <Section title="NFT Rental Vault" sub="ERC-4626 vault · stake a pony NFT for boosted yield">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <StatCard
              label="Total Assets"
              value={totalAssets !== undefined ? formatAmount(totalAssets as bigint) : "—"}
              sub="GOV locked"
            />
            <StatCard
              label="Your Shares"
              value={shares !== undefined ? formatAmount(shares as bigint) : "—"}
              sub="vSHARE"
              accent
            />
            <StatCard
            ...
            <StatCard
              label="Your GOV"
              value={govBalance !== undefined ? formatAmount(govBalance as bigint) : "—"}
              sub="Available"
            />
          </div>

          {/* NFT Boost panel */}
          <div className={`card border ${isRenting ? "border-orange/40 bg-orange/5" : "border-border"}`}>
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="font-display font-semibold text-white text-sm">Pony NFT Boost</p>
                <p className="text-subtext text-xs font-mono mt-0.5">
                  Stake Pinkie Pie (#{TOKEN_IDS.PINKIE_PIE.toString()}) for {boostMultiplier.toFixed(1)}x yield
                </p>
              </div>
              {isRenting
            ...
            <div className="flex items-center gap-3">
              <span className="text-subtext text-xs font-mono">
                Pinkie Pie balance: {pinkieBalance !== undefined ? (pinkieBalance as bigint).toString() : "—"}
              </span>
              {isRenting ? (
                <TxButton onClick={handleUnstakeNFT} loading={loading} variant="ghost">
                  Unstake NFT
                </TxButton>
              ) : (
                <TxButton
            ...
            {mode === "deposit" ? (
              <>
                <InputField
                  ...
                  placeholder="0.0"
                  type="number"
                  hint={isRenting ? `With boost: ~${(Number(amount || 0) * boostMultiplier).toFixed(4)} shares` : undefined}
                />
                {isSuccess ? (
              ...
              <>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-subtext font-mono">Shares to redeem</span>
                  <span className="font-mono text-white">
                    {shares !== undefined ? formatAmount(shares as bigint) : "—"} vSHARE
                  </span>
                </div>
                {isSuccess ? (
                  <div className="bg-green/10 border border-green/40 text-green text-sm font-mono rounded-lg p-3 text-center">
                  ...
                  <TxButton
                    ...
                    loading={loading}
                    disabled={(shares as bigint ?? 0n) === 0n}
                    variant="ghost"
                  >
                    Redeem All Shares
                  </TxButton>
